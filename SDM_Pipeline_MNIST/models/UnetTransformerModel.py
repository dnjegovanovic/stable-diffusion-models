...
class UnetTransformerModel(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.conv_1 = nn.Conv2d(1, channels[0], 3, stride=1, bias=False)
        self.dense_1 = RODense(embed_dim, channels[0])
        self.gnorm_1 = nn.GroupNorm(4, num_channels=channels[0])

        self.conv_2 = nn.Conv2d(channels[0], channels[1], 3, stride=2, bias=False)
        self.dense_2 = RODense(embed_dim, channels[1])
        self.gnorm_2 = nn.GroupNorm(32, num_channels=channels[1])

        self.conv_3 = nn.Conv2d(channels[1], channels[2], 3, stride=2, bias=False)
        self.dense_3 = RODense(embed_dim, channels[2])
        self.gnorm_3 = nn.GroupNorm(32, num_channels=channels[2])
        self.attn_3 = SpatialTransformer(channels[2], text_dim)

        self.conv_4 = nn.Conv2d(channels[2], channels[3], 3, stride=2, bias=False)
        self.dense_4 = RODense(embed_dim, channels[3])
        self.gnorm_4 = nn.GroupNorm(32, num_channels=channels[3])
        ...
        self.dense_6 = RODense(embed_dim, channels[1])
        self.tgnorm_3 = nn.GroupNorm(32, num_channels=channels[1])

        self.tconv_2 = nn.ConvTranspose2d(
            channels[1], channels[0], 3, stride=2, bias=False, output_padding=1
        )  #  + channels[1]
        self.dense_7 = RODense(embed_dim, channels[0])
        self.tgnorm_2 = nn.GroupNorm(32, num_channels=channels[0])
        self.tconv_1 = nn.ConvTranspose2d(channels[0], 1, 3, stride=1)  #  + channels[0]
    ...
    def forward(self, x: torch.Tensor, time_feature: torch.Tensor, y=None):
        ...
        # Decoding path
        h = self.tconv_4(h4) + self.dense_5(embed)
        ## Skip connection from the encoding path
        h = self.act(self.tgnorm_4(h))
        h = self.tconv_3(h + h3) + self.dense_6(embed)
        h = self.act(self.tgnorm_3(h))
        h = self.tconv_2(h + h2) + self.dense_7(embed)
