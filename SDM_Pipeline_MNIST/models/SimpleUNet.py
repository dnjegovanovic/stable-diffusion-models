...
class SimpleUNet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.time_embed = nn.Sequential(GFP(embeded_dim=self.embed_dim),
        ...
        self.conv_2 = nn.Conv2d(self.chanels[2], self.chanels[1], kernel_size=3, stride=2, bias=False)
        self.ro_dense_2 = RODense(self.embed_dim, self.chanels[1])
        self.gnorm_2 = nn.GroupNorm(32, num_channels=self.chanels[1])
        
        self.conv_3 = nn.Conv2d(self.chanels[1], self.chanels[2], kernel_size=3, stride=2, bias=False)
        self.ro_dense_3 = RODense(self.embed_dim, self.chanels[2])
        self.gnorm_3 = nn.GroupNorm(32, num_channels=self.chanels[2])
        
        self.conv_4 = nn.Conv2d(self.chanels[2], self.chanels[3], kernel_size=3, stride=2, bias=False)
        ...
        self.trans_conv_4 = nn.ConvTranspose2d(self.chanels[3], self.chanels[2], kernel_size=3, stride=2, bias=False)
        self.ro_dense_5 = RODense(self.embed_dim, self.chanels[2])
        self.trans_gnorm_4 = nn.GroupNorm(32, num_channels=self.chanels[2])
        
        self.trans_conv_3 = nn.ConvTranspose2d(self.chanels[2]+self.chanels[2], self.chanels[1], kernel_size=3, stride=2, bias=False, output_padding=1)
        self.ro_dense_6 = RODense(self.embed_dim, self.chanels[1])
        self.trans_gnorm_3 = nn.GroupNorm(32, num_channels=self.chanels[1])
        
        self.trans_conv_2 = nn.ConvTranspose2d(self.chanels[1]+self.chanels[1], self.chanels[0], kernel_size=3, stride=2, bias=False, output_padding=1)
        self.ro_dense_7 = RODense(self.embed_dim, self.chanels[0])
        self.trans_gnorm_2 = nn.GroupNorm(32, num_channels=self.chanels[0])
        
        self.trans_conv_1 = nn.ConvTranspose2d(self.chanels[0] + self.chanels[0],1, kernel_size=3,stride=1)
